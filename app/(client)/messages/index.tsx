...
export default function ClientMessagesScreen() {
  ...
  const handleConversationPress = (contractId: string) => {
    router.push(`/(client)/chat/${contractId}` as any);
  };

  const renderConversation = ({ item }: { item: Contract }) => {
    const unreadCount = unreadCounts[item._id] || 0;
    // For client, show freelancer name
    const partnerName = item.freelancerDisplayName || "Freelancer";
    ...
    return (
      <TouchableOpacity
        ...
        activeOpacity={0.7}
      >
        <Avatar name={partnerName} size="md" variant="freelancer" />
