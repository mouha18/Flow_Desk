...
| 4 | Wire accept email send into `contracts:accept` mutation | `feat` | `convex/contracts.ts` | 📋 |
| 5 | Implement `convex/actions/push.ts` — Expo Push API sender | `feat` | `convex/actions/push.ts` | 📋 |
| 6 | Wire push sends into all notification-triggering mutations | `feat` | `convex/contracts.ts`, `convex/tasks.ts` | 📋 |
| 7 | Implement `convex/invoices.ts` — generate (action), getByContract, update, send, simulatePayment | `feat` | `convex/invoices.ts`, `convex/actions/ai.ts` | 📋 |
| 8 | Build `useInvoice` hook | `feat` | `hooks/useInvoice.ts` | 📋 |
| 9 | Build Invoice Draft screen (editable line items, tax, total) | `feat` | `app/(freelancer)/contracts/[id]/invoice.tsx`, `components/invoice/*` | 📋 |
| 10 | Build Invoice View screen for client | `feat` | `app/(client)/contracts/[id]/invoice.tsx` | 📋 |
| 11 | Build Payment Simulation screen (Stripe / NabooPay UI mock) | `feat` | `app/(client)/contracts/[id]/invoice.tsx` | 📋 |
| 12 | Wire payment confirmation → deliverable link release + emails + push | `feat` | `convex/invoices.ts`, `convex/actions/email.ts`, `convex/actions/push.ts` | 📋 |
| 13 | Add deliverable link field to Contract Detail (freelancer sets before sending invoice) | `feat` | `app/(freelancer)/contracts/[id]/index.tsx` | 📋 |
| 14 | Add `lib/formatting.ts` (currency XOF/USD, duration mm:ss, date) | `chore` | `lib/formatting.ts` | 📋 |
...
- [ ] "Generate Invoice" appears at 100%; Anthropic returns valid invoice JSON
...
| 4 | Polish Freelancer dashboard (contract stats, active count, completion bars) | `feat` | `app/(freelancer)/dashboard/index.tsx` | 📋 |
| 5 | Polish Client dashboard (pending badge count, active project card) | `feat` | `app/(client)/dashboard/index.tsx` | 📋 |
| 6 | Add unread notification badge to Tab navigator | `feat` | Navigator layouts | 📋 |
| 7 | Add deep link navigation from notification tap to correct contract screen | `feat` | `hooks/usePushNotifications.ts`, navigator | 📋 |
| 8 | Add profile screen (both roles: name, pseudo, edit, logout) | `feat` | `app/(freelancer)/profile/index.tsx`, `app/(client)/profile/index.tsx` | 📋 |
| 9 | Final SQLite sync verification (offline read confirmed) | `chore` | `lib/sqlite.ts` | 📋 |
| 10 | End-to-end demo run (create contract → full cycle → deliverable received) | `chore` | — | 📋 |
| 11 | Bug fixes from demo run | `chore` | TBD | 📋 |
| 12 | README + .env.example finalized | `chore` | `README.md`, `.env.example` | 📋 |

**Definition of Done:**
- [ ] Every screen has an empty state
- [ ] No screen shows a white flash or undefined crash on load
- [ ] Notification tap deep links to the correct contract screen
- [ ] Full lifecycle demo runs without error in one take
- [ ] All 3 navigation types (Stack, Tab, Drawer) clearly demonstrated
- [ ] SQLite cache confirmed working by testing in airplane mode
- [ ] Push notification received on physical device during demo
...
- [ ] AI invoice generation — Sprint 3
- [ ] Invoice draft editing — Sprint 3
- [ ] Invoice send — Sprint 3
- [ ] Payment simulation — Sprint 3
- [ ] Deliverable link release on payment — Sprint 3
- [ ] Empty states + loading states — Sprint 4
- [ ] Deep link from notification — Sprint 4
- [ ] Profile screen — Sprint 4
- [ ] Demo-ready end-to-end flow — Sprint 4
